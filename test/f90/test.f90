program test
    ...
    ! Variables to read
    !-------------------
    integer:: iyr(n_iter) ! Reading just the years for testing purposes 
    real:: sp(n_points, n_iter)
    
    ! File path 
    !------------
    character (len=*), parameter:: FILE_NAME =trim("/home/aredjil/projects/OASIM_ATM/data000.nc")
    ...
    ! write(*, *) " "
    ! write(*, *) " "
    ! The below line was commented before me commenting leave it comented 
    ! write(*, *) lib%m_size
    ! write(*, *) " "
    ! do i = 1, lib%m_size
    ...
    ! Reading the sp variables 
    !-------------------------
    call check(nf90_inq_varid(ncid, "sp", varid))
    call check(nf90_get_var(ncid, varid, sp))
    ! Prinitng the year for testing 
    !------------------------------
    print *, "The years in the file are: "
    print *, iyr 
